 ...
 * @property {() => string} getFixPrompt
 *   The user's current "Fix this PDF" instructions (their own, or the
 *   default in lib/fix-document-prompt.txt). Asked for on every use rather
 *   than captured once, so an edit in Preferences applies to the next request.
...
function buildMcpServer(deps) {
  const server = new McpServer(
    { name: 'lastmilepdf', version: deps.version },
    {
      instructions:
        ...
        + 'When asked to fix, clean up or remediate the document as a whole, call get_remediation_instructions before anything else and follow them: they are the user\'s own standing instructions for that job. '
        + NODE_ID_NOTE,
    },
  );
  ...
  // "Fix this PDF", two ways in. The prompt is for a user who picks it from
  // their client (in Claude Code, /mcp__lastmilepdf__fix_document); the tool
  // is for one who just types "fix this" - the server instructions above
  // send Claude to it - and for clients that don't surface MCP prompts.
  server.registerPrompt('fix_document', {
    title: 'Fix this PDF',
    description: 'Remediate the whole open document for final use, following the instructions set in LastMilePDF (File > Settings > Preferences > Claude connection).',
    argsSchema: {
      notes: z.string().optional().describe('Anything particular to this document, e.g. "this is one chapter of a book" or "leave the appendix alone".'),
    },
  }, ({ notes }) => {
    let text = deps.getFixPrompt();
    if (notes && notes.trim()) text += `\n\nNotes from the user about this document:\n${notes.trim()}`;
    return {
      description: 'Remediate the open PDF',
      messages: [{ role: 'user', content: { type: 'text', text } }],
    };
  });

  server.registerTool('get_remediation_instructions', {
    title: 'The user\'s instructions for fixing a whole document',
    description:
      'The instructions the user has set for fixing, cleaning up or remediating a whole document - their "Fix this PDF" prompt, which they can edit in the app\'s Preferences. '
      + 'Call this first whenever you are asked to fix or remediate the document as a whole, and follow it over your own defaults. Not needed for a request about one tag or one table.',
    inputSchema: {},
    annotations: READ_ONLY,
  }, guarded(async () => ({ content: [{ type: 'text', text: deps.getFixPrompt() }] })));

  server.registerTool('get_app_status', {
    title: 'App status',
    description: 'Whether a PDF is open in the app, and if so its file name, page count, whether it is tagged, whether it has unsaved changes, its title, author and language, and how many bookmarks it has.',
    inputSchema: {},
    annotations: READ_ONLY,
  ...
  server.registerTool('get_nodes', {
    ...
    inputSchema: {
      ...
      includeLeaves: z.boolean().optional().describe('Also list each tag\'s content leaves - the pieces of page content it holds directly - with id, text and position among its children. Default false (only a count is given): an OCR\'d paragraph can be dozens. Needed for wrap_content and split_content.'),
    },
    annotations: READ_ONLY,
  ...
  server.registerTool('wrap_content', {
    ...
    description:
      ...
      + 'Works at the level of whole leaves: to take only part of one, cut it first with split_content. LI builds its LBody; L and Table are refused - wrap the items or cells, then group them with apply_tag_action.',
    inputSchema: {
      revision: REVISION,
  ...
  server.registerTool('split_content', {
    title: 'Split content leaves',
    description:
      'Cuts content leaves in two or more pieces, each cut named by the text it goes BEFORE - how to separate a list label from its item ("1.)" | "Blah"), a run-in heading from its paragraph, or two paragraphs OCR ran together. Then tag the pieces with wrap_content. '
      + 'Matching ignores whitespace, and each cutBefore must occur exactly once in the rest of the leaf (after the previous cut) - give a longer piece if it repeats. '
      + 'The text you get from get_nodes comes from the page preview and can differ slightly from what the file encodes; if a cut isn\'t found, the refusal quotes the leaf\'s exact text to retry against. '
      + 'Refused, changing nothing, if any cut can\'t be made: a leaf whose font has no text mapping, a cut inside a ligature. '
      + 'Unlike other edits this rewrites the page\'s content (the marked-content boundaries, not what is drawn). Returns each piece\'s new id and text. Renumbers ids from the earliest leaf onward.',
    inputSchema: {
      revision: REVISION,
      splits: z.array(z.object({
        leafId: z.string(),
        cutBefore: z.array(z.string().min(1)).min(1).max(50).describe('Texts to cut before, in reading order within the leaf.'),
      }).strict()).min(1).max(200),
    },
    annotations: EDIT,
  }, viaRenderer('splitContent'));
  ...
  server.registerTool('generate_bookmarks', {
    title: 'Generate bookmarks from headings',
    description:
      'The Bookmarks panel\'s Generate button: REPLACES the whole outline (every existing bookmark) with one built from the H1-H6 heading tags, nested by level, each titled with the heading\'s Actual Text or else its text and pointing at where it sits on the page. '
      + 'So get the headings right first - their roles and levels decide the outline. Refused, changing nothing, if the document has no H1-H6 tags. Returns the new outline.',
    inputSchema: {},
    annotations: { ...EDIT, destructiveHint: true },
  }, viaRenderer('generateBookmarks'));

  server.registerTool('set_document_properties', {
    title: 'Set title, author and language',
    description:
      'Sets the document\'s own metadata, as the Title / Author / Document language fields do when the user selects the document root: title and author (written to both the Info dictionary and XMP), and the primary language (the catalog /Lang, e.g. "en-US"). '
      + 'Setting a title also makes viewers show it instead of the file name, which the accessibility checks require. Give only the fields to change; an empty string clears one. Current values are in get_app_status.',
    inputSchema: {
      title: z.string().max(1000).optional(),
      author: z.string().max(1000).optional(),
      language: z.string().max(35).optional().describe('A BCP 47 language tag such as "en-US" or "fr".'),
    },
    annotations: { ...EDIT, idempotentHint: true },
  }, viaRenderer('setDocumentProperties'));
