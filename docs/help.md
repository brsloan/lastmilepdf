...
: Ask Claude to "fix this PDF" and it works through the whole document following a standing set of instructions: survey every page first, remove organisational wrappers, set one consistent heading outline, join lists and tables broken across pages, put captions where they belong, give every figure alt text, mark likely artifacts as **Artifact?** for you to review, set the title and author, generate bookmarks, and end with a report of what it did and what still needs you. In Claude Code the same instructions are also a command, `/mcp__lastmilepdf__fix_document`, which takes an optional note about the document ("this is one chapter of a book"). The instructions are yours to change: File > Settings > Preferences > **Claude connection** > *Instructions for "Fix this PDF"*. Write your own house rules there; **Reset to default** brings back the original. Changes apply the next time you ask. Transcription errors across whole paragraphs are still a job for Fix All Actual Text (AI), which Claude will tell you to run.
