...
- Use AI to clean up OCR errors in Actual Text fields, highlighting changes for approval (configurable to use any AI provider). Fixing a single tag also sends the AI a crop of the page the text came from, so it corrects against the scan rather than guessing at names and numbers
- Tag figures on scanned pages that were missed by auto-tagger
- Fill with AI on a selected Figure or Formula sends the AI an image of just that tag's part of the page and writes the result into the Alt text field as a first draft to review - a description of the figure, or the formula read out in words (needs a provider and model that can read images)
- Select Content: drag a rectangle over the page preview to select the text under it and tag it with a single keystroke - the rectangle cuts leaves at its own edges, so you can tag half a paragraph as a heading, or a set of bullets as a real list with a Lbl/LBody per item, without touching the tag tree. Ctrl+L does the same for a reference list, whose entries are marked out by a hanging indent rather than by any character in the text. T lays a table grid over the selection instead: the column and row dividers start where the text's gaps suggest them, you correct them by clicking and dragging (G guesses again, Delete clears), merge cells and mark headers, and Enter builds the Table/TR/TH/TD structure with every leaf cut to its cell. A (or Try with AI) asks the configured AI provider to read the table from an image of the box and its words instead, and replaces the grid with its reading - spans and headers included - for you to check before building
...
renderer/ (Chromium, no Node access)
  ...
  rect-select.js  - Select Content: the rubber-band rectangle, the glyph
                    ...
                    hands the worker
  table-grid.js   - Select Content -> Table: the grid of dividers and cells
  table-seed.js   - guesses that grid's dividers from the text's geometry (no DOM; tested by scripts/table-seed-test.js)
                    drawn over a rectangle before it becomes a Table tag
...
preload.js      - contextBridge: exposes window.api.{openPdf,updateNode,updateNodes,
                  ...
                  savePdf,saveToPath,getPageCodeBoxes,tagRectContent,tagRectTable,wrapLeaves,
                  onMenu*}
